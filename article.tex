...
implemented, and tested. The Sec.~\ref{sec:Requs} of
...
The Sec.~\ref{sec:lexical} briefly outlines lexical rules of the language.
The Sec.~\ref{sec:syntax} describes the syntax using extended \nospell{Backus-Naur}
form and gives a few practical examples. The Sec.~\ref{sec:semantic} outlines
a semantic layer of the compiler that is intended to detect vast majority
of inconsistencies in a requirements document.
The Sec.~\ref{sec:uml} gives an example of to-UML mapping implemented
by the compiler.
The Sec.~\ref{sec:ambiguity}
...
The Sec.~\ref{sec:results} presents empirical results obtained from three
commercial Java web projects over the last few years.
The Sec.~\ref{sec:conclusion} presents conclusions and observations made by
the author.
...
In the Fig.~\ref{fig:example}, the entities \ff{SuD}, \ff{Fraction}, \ff{Float}, and
...
The second line in the Fig.~\ref{fig:example} declares the class \ff{Fraction}
...
the Fig.~\ref{fig:example}, method ``calculates'' of the class \ff{Fraction} may
