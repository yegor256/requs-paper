...
\usepackage{tikz}
  ...
  \usetikzlibrary{calc}
\usepackage{href-ul} % hyper links
...
business people. The Fig.~\ref{fig:example} presents an example of a short
...
\ff{Float} is a built-in class, as well as \ff{Text}, \ff{Integer}, and
...
In the example, use case \ff{UC1} is a method of class \ff{SuD} and its
...
\ff{1} of \ff{UC1} is creating a new object \ff{fraction} of class
\ff{Fraction}, while step \ff{2} is calling a method ``calculates'' of
\ff{fraction} in order to get back an object named \ff{quotient} of
class \ff{Float}.
...
The Fig.~\ref{fig:java} contains a close equivalent program in Java.
...
\end{ffcode}
\caption{Java equivalent of Requs requirements document from the Fig.~\ref{fig:example}}
\label{fig:java}
\end{figure}
...
In the Fig.~\ref{fig:example}, \ff{SuD}, \ff{Fraction}, \ff{Float}, and
\ff{User} are classes. They start with capital letters and contain only
letters (both lowercase and capital). \ff{user}, \ff{fraction}, and
\ff{quotient} are variables bound to instances/objects of classes.
...
\ff{requires}, etc. (A full list of them is defined below in Section~\ref{sec:syntax}.)
...
The second line in the the Fig.~\ref{fig:example} declares class \ff{Fraction}
...
\ff{Fraction} class. Class \ff{User} don't have any arguments. Class
\ff{SuD} is a pre-defined class without arguments. Class \ff{Float} is a
pre-defined class with one argument.

Class without arguments in constructor normally means that it represents an
entity outside of system scope, like \ff{User}.
...
SRS starting with \ff{UC} (use case). In other words,
...
than \ff{Float} would be rejected by the compiler.
...
expects method ``calculates'' of class \ff{Fraction} to return an object
...
returns an instance of class \ff{Float}. Otherwise, the compiler would
...
build-in method \ff{Fail since}. The method has one parameter of class
\ff{Text}, which explains the reason. In the example in
the Fig.~\ref{fig:example}, method ``calculates'' of class \ff{Fraction} may
...
class \ff{SuD} that encapsulates other objects and methods. This object
hierarchy is convertible to UML diagrams~\citep{uml}. Specification in
the Fig.~\ref{fig:example} would be converted to the following UML diagrams
(to save space, the list is limited to the three most interesting
diagrams):

\begin{itemize}
\item Class diagram, in the Fig.~\ref{fig:uml-class}
\item Object diagram, in the Fig.~\ref{fig:uml-object}
\item Sequence diagram of ``divides two numbers'' method, in the Fig.~\ref{fig:uml-seq}
\end{itemize}
...
other object-oriented language. The Fig.~\ref{fig:xmi} shows an example XMI
